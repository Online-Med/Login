    ...
    if (method === 'POST') {
      ...
      if (body.action === 'bloquear_faixa') {
        ...
        for (let m = parseMin(hora_inicio); m < parseMin(hora_fim); m += inv) {
          bloqueios.push({
            id_profissional: parseInt(id_profissional), data_agenda,
            hora_agenda: fmtH(m), paciente_nome: 'BLOQUEADO',
            tipo: 'BLOQUEIO', modalidade: 'PRESENCIAL', status: 'BLOQUEADO',
            primeira_consulta: false, is_encaixe: false, is_bloqueado: true,
